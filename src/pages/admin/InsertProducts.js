...
const getAllProducts = async () => {
  try {
      const response = await fetch(
        `/api/products?id=${state.adminData.ID}`,
        {method:'GET'}
        );
        ...
        <form id='Form-Insert-Product' onSubmit={(e)=>{insertProduct(e)}} className="">
          ...
          <div className="flex flex-wrap -mx-3 mb-6">
            <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
              ...
              <input onChange={(e)=>setProduct({...product,...{title:e.target.value}})} className="appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="title" type="text" placeholder="Title" required/>
  ...
  <div>
    ...
    export default connect(mapStateToProps,null)(InsertProducts);
